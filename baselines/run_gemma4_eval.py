#!/usr/bin/env python3
"""
gemma4(本地开源) 数据评估 (V2, P6)
=========================
对 gemma4 采集的样本, 用 evaluate_baselines.ts 跑 7 方法, 输出恢复率/F1/值精确率。
...
GT = ROOT/"data"/"gemma4"/"ground_truth.json"
...
def main():
    ...
    print(f"gemma4 samples: {len(gt)} gt, {len(samples)} trunc samples")
    nr=run_node(samples)
    ...
    outpath=ROOT/"results"/"gemma4_eval.json"
    outpath.write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding="utf-8")
    print(f"\nSaved {outpath}")
    # 日志
    record("gemma4_eval", params={"model":"gemma4(local/ollama/Metal)","n_per_schema":a.n_per_schema,"parser":"fixed"},
           inputs={"gt_json":str(GT),"script":str(SCRIPT)}, outputs={f.name:"" for f in [outpath]},
           metrics=summary, notes="gemma4 local samples, 同V8沙箱, 架构无关性验证")
